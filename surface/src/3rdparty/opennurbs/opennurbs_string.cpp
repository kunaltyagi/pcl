...
void
ON_String::EnableReferenceCounting (bool bEnable)
{
  // TODO fill this in;
}
...
void
ON_String::ReserveArray (size_t array_capacity)
{
  ON_aStringHeader *p = Header ();
...
void
ON_String::SetLength (size_t string_length)
{
...
ON_String::Remove (const char chRemove)
{
  ...
  *pstrDest = 0;
  int nCount =
      (int)(pstrSource - pstrDest); // the (int) is for 64 bit size_t conversion
...
bool
ON_CheckSum::SetBufferCheckSum (size_t size, const void *buffer, time_t time)
{
  bool rc = false;
  Zero ();
  if (size != 0 && buffer != 0) {
...
ON_CheckSum::SetFileCheckSum (FILE *fp)
{
  ...
  if (fp) {
    ...
    while (1024 == count) {
      count =
          (int)fread (buffer, 1, 1024, fp); // the (int) is for 64 bit size_t conversion
      if (count > 0) {
        m_size += count;
        crc = ON_CRC32 (crc, count, buffer);
      }
...
bool
ON_CheckSum::CheckBuffer (size_t size, const void *buffer) const
{
  if (m_size != size)
    return false;
  if (0 == size)
