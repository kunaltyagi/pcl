...
bool
ON_BinaryArchive::ReadStringUTF8ElementCount (size_t *string_utf8_element_count)
{
  ON__UINT32 ui32 = 0;
  bool rc = ReadInt32 (1, (ON__INT32 *)&ui32);
  // Note that ui32 = number of elements in the string array, including
...
ON_BinaryArchive::ReadString (         // Read NULL terminated unicode string
    size_t string_utf16_element_count, // length = value ReadStringUTF16ElementCount
                                       // ReadStringSize()
...
ON_BinaryArchive::ReadString (ON_String &s)
{
  ...
  if (rc && string_utf8_element_count > 0) {
    const int istring_utf8_element_count =
        (int)string_utf8_element_count; // (int) converts 64 bits size_t
...
bool
ON_BinaryArchive::WriteBigSize (size_t sz)
{
...
bool
ON_BinaryArchive::ReadBigSize (size_t *sz)
{
  ON__UINT64 u;
...
)
{
  ...
  if (ReadMode ()) {
    ...
    if (c) {
      ...
      rc = BigSeekBackward (
          pos0 -
          c->m_big_offset); // pos0 >= c->m_offset, so this size_t subtraction is ok
    } else {
...
ReadV1_TCODE_LEGACY_SRFSTUFF (ON_BinaryArchive &file)
{
  ...
  int i, j, dim = 0, is_rat = 0, order[2], cv_count[2], is_closed[2], is_singular[2],
            form;
  ON_BoundingBox bbox;
  char c;
...
bool
ON_BinaryArchive::Read3dmEndMark (size_t *file_length)
{
  unsigned int tcode = 0;
  ON__INT64 value = 0;
  if (file_length)
...
void
ON_BinaryArchive::UpdateCRC (size_t count, const void *p)
{
  if (m_bDoChunkCRC) {
    ON_3DM_BIG_CHUNK *c = m_chunk.Last ();
    if (c) {
...
size_t
ON_BinaryFile::Read (size_t count, void *p)
{
  return (m_fp) ? fread (p, 1, count, m_fp) : 0;
}

size_t
ON_BinaryFile::Write (size_t count, const void *p)
{
  size_t rc = 0;
  if (m_fp) {
    if (m_memory_buffer) {
...
bool
ON_BinaryFile::SeekFromStart (size_t offset)
{
  bool rc = false;
  if (m_fp) {
...
static void
Dump3dmChunk_ErrorReportHelper (size_t offset, const char *msg, ON_TextLog &dump)
{
  int ioffset = (int)offset;
...
bool
ON_Read3dmBufferArchive::SeekFromStart (size_t offset)
{
  bool rc = false;
  if (m_buffer) {
    if (offset > 0)
...
size_t
ON_Read3dmBufferArchive::Read (size_t count, void *buffer)
{
  if (count <= 0 || 0 == buffer)
    return 0;
...
size_t
ON_Read3dmBufferArchive::Write (size_t, const void *)
{
...
void
ON_Write3dmBufferArchive::AllocBuffer (size_t sz)
{
...
bool
ON_Write3dmBufferArchive::SeekFromStart (size_t offset)
{
  bool rc = false;
  if (m_buffer) {
    if (offset > 0)
...
size_t
ON_Write3dmBufferArchive::Read (size_t count, void *buffer)
{
  if (count <= 0 || 0 == buffer)
    return 0;
...
size_t
ON_Write3dmBufferArchive::Write (size_t sz, const void *buffer)
{
  if (sz <= 0 || 0 == buffer)
    return 0;
...
bool
ON_BinaryArchiveBuffer::SeekFromStart (size_t offset)
{
  if (0 != m_buffer)
...
size_t
ON_BinaryArchiveBuffer::Read (size_t count, void *a)
{
  if (0 != m_buffer)
...
size_t
ON_BinaryArchiveBuffer::Write (size_t count, const void *a)
{
  if (0 != m_buffer)
...
ON_FileIterator::ON_FileIterator ()
    : m_count (0)
      ...
      m_current_file_create_time (0), m_current_last_modified_time (0),
