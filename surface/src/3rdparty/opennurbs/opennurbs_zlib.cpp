...
#pragma comment(lib,                                                                   \
                "\"" OPENNURBS_ZLIB_OUTPUT_ROOT_DIR "/" OPENNURBS_CONFIGURATION_DIR    \
                "/" OPENNURBS_ZLIB_FILE_NAME "\"")
...
bool
ON_BinaryArchive::ReadCompressedBufferSize (size_t *sizeof__outbuffer)
{
...
bool
ON_BinaryArchive::ReadInflate (size_t sizeof___outbuffer, // sizeof uncompressed data
...
)
{
  ...
  bool bValidCompressedBuffer = false;
  {
    ...
    if (tcode == TCODE_ANONYMOUS_CHUNK && big_value > 4 && sizeof___outbuffer > 0 &&
      ...
      sizeof__inbuffer =
          (size_t) (big_value - 4); // the last 4 bytes in this chunk are a 32 bit crc
...
bool
ON_CompressedBuffer::WriteChar (size_t count, const void *buffer)
{
  bool rc = true;
  if (count > 0 && buffer) {
