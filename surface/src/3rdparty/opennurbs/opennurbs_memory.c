...
void *
onmalloc_from_pool (ON_MEMORY_POOL *pool, size_t sz)
{
  void *p;
  p = (sz > 0) ? malloc (sz) : 0;
  return p;
...
void *
oncalloc_from_pool (ON_MEMORY_POOL *pool, size_t num, size_t sz)
{
  void *p;
...
void *
oncalloc (size_t num, size_t sz)
{
  return oncalloc_from_pool (0, num, sz);
}

void
onfree (void *memblock)
{
  if (0 != memblock)
    free (memblock);
}
...
void *
onrealloc (void *memblock, size_t sz)
{
  return onrealloc_from_pool (0, memblock, sz);
}

void *
onrealloc_from_pool (ON_MEMORY_POOL *pool, void *memblock, size_t sz)
{
  void *p;
