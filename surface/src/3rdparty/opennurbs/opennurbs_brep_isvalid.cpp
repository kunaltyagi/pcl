  ...
  if (first_trim_ti >= 0 && first_trim_vi0 != prev_trim_vi1) {
    ...
    if (text_log) {
      ...
      text_log->Print (
          ...
          loop.m_ti.Count () - 1, prev_trim_ti, prev_trim_vi1, first_trim_ti,
          first_trim_vi0);
      text_log->PopIndent ();
    }
    return false;
...
ON_Brep::IsValidTrimTopology (int trim_index, ON_TextLog *text_log) const
{
  ...
  if (trim.m_type == ON_BrepTrim::singular) {
    ...
    if (trim.m_vi[0] != trim.m_vi[1]) {
      if (text_log)
        text_log->Print ("ON_Brep.m_T[%d].m_type = singular, but m_vi = (%d,%d) "
                         "(should be same vertex index).\n",
