...
ON_Buffer::ON_Buffer ()
    : m_buffer_size (0), m_current_position (0), m_first_segment (0),
      m_last_segment (0), m_current_segment (0), m_heap (0), m_error_handler (0),
      m_last_error (0)
{
...
ON_Buffer::ON_Buffer (const ON_Buffer &src)
    : m_buffer_size (0), m_current_position (0), m_first_segment (0),
      m_last_segment (0), m_current_segment (0), m_heap (src.m_heap),
      m_error_handler (0), m_last_error (0)
{
  memset (m_reserved, 0, sizeof (m_reserved));
  Copy (src);
}
      ...
      struct ON_BUFFER_SEGMENT *new_last_segment =
          (struct ON_BUFFER_SEGMENT *)onrealloc (
              ...
              sizeof (*m_last_segment) +
                  ((size_t)sizeof_segment_buffer)); // sizeof_segment_buffer always <
                                                    // 0xFFFFFFFF
...
bool
ON_Buffer::IsValid (const ON_TextLog *text_log) const
{
...
ON_Buffer::CRC32 (ON__UINT32 current_remainder) const
{
  ...
  for (seg = m_first_segment; 0 != seg; seg = seg->m_next_segment) {
    ...
    current_remainder =
        ON_CRC32 (current_remainder, (size_t)seg_size, seg->m_segment_buffer);
    size += seg_size;
...
ON_Buffer::Write (ON__UINT64 size, const void *buffer)
{
  ...
  while (size > 0) {
    if (0 == m_current_segment) {
      ...
      // (size_t) cast is safe because malloc_size is always <= max_malloc_size =
...
ON_EmbeddedFile::Extract (void *out_buffer) const
{
  ...
  for (;;) {
    ...
    if (m_bCompressedBuffer) {
      ...
      file_crc = ON_CRC32 (file_crc, (size_t)buffer_size, buffer);
      size_t sz =
          (bs.buffer_size < buffer_size) ? (size_t)bs.buffer_size : (size_t)buffer_size;
      if (sz > 0) {
        memcpy (bs.buffer, buffer, sz);
        bs.buffer_size -= sz;
        bs.buffer = ((unsigned char *)bs.buffer) + sz;
