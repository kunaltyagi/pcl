...
void
ON_HatchExtra::Dump (ON_TextLog &text_log) const
{
}
...
void *
ON_HatchLoop::operator new (size_t sz)
{
  // ON_HatchLoop new
  return onmalloc (sz);
}
...
void *
ON_HatchLoop::operator new (size_t, void *p)
{
...
ON_Hatch::ON_Hatch (const ON_Hatch &src)
    : ON_Geometry (src), m_plane (src.m_plane), m_pattern_scale (src.m_pattern_scale),
      m_pattern_rotation (src.m_pattern_rotation), m_pattern_index (src.m_pattern_index)
{
