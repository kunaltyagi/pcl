...
ON_UserData::IsValid (ON_TextLog *text_log) const
{
  ...
  if (0 == ON_UuidCompare (m_userdata_uuid, ON_UserData::ClassId ()->Uuid ())) {
    if (0 != text_log) {
      text_log->Print ("invalid userdata - m_userdata_uuid in use. Use guidgen to get "
                       "a unique id.\n");
    }
    return false;
  }
...
bool
ON_UnknownUserDataArchive::SeekFromStart (size_t offset)
{
  bool rc = false;
...
size_t
ON_UnknownUserDataArchive::Read (size_t count, void *buffer)
{
  size_t maxcount = 0;
...
size_t
ON_UnknownUserDataArchive::Write (size_t, const void *)
{
