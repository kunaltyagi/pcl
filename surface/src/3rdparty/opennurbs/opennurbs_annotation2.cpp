...
void
ON_TextExtra::Dump (ON_TextLog &text_log) const
{
  // do nothing
}
...
void
ON_DimensionExtra::Dump (ON_TextLog &text_log) const
{
  // do nothing
}
...
void
ON_AngularDimension2Extra::Dump (ON_TextLog &text_log) const
{
  // do nothing
}
...
ON_AngularDimension2::GetArc (ON_Arc &arc) const
{
  ...
  if (ON_IsValid (m_radius) && m_radius > ON_SQRT_EPSILON && ON_IsValid (m_angle) &&
      ...
      m_plane.xaxis.IsValid () && m_plane.yaxis.IsValid () &&
...
ON_AngularDimension2::ConvertBack (
    ON_AngularDimension2
        & // target - formal parameter intentionally ignored in this virtual function
)
{
}
                              ...
                              ON_Xform &xform) const
{
  ...
  //    text_centering_rotation.y*m_plane.yaxis; x = V*cameraX; y = V*cameraY; if (
  //    fabs(x) <= ON_SQRT_EPSILON && fabs(y) > ON_SQRT_EPSILON )
  //    {
  //      x = y;
  //    }
  //    if ( x < 0.0 )
