    ...
    for (int i = 0; i < num_yaw; i++) {
      for (int j = 0; j < num_pitch; j++) {
        if (yaw_pitch_bins[i][j] >= min_images_per_bin_) {
          std::random_shuffle (image_files_per_bin[i][j].begin (),
                               image_files_per_bin[i][j].end ());
                             ...
                             std::vector<ExampleIndex> &examples)
{
  ...
  for (size_t j = 0; j < files.size (); j++) {
    ...
    for (int jjj = 0; jjj < static_cast<int> (cloud->points.size ()); jjj++) {
      cloud_intensity->points[jjj].getVector4fMap () =
          cloud->points[jjj].getVector4fMap ();
    ...
    //************************************************
    {
      ...
      for (const auto &p : positive_p) {
        ...
        te.trans_ =
            center_point.getVector3fMap () - patch_center_point.getVector3fMap ();
