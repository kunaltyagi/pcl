...
namespace pcl
{
  ...
  class PCL_EXPORTS CRHAlignment
  {
                      ...
                      std::vector<float> &peaks)
    {
      ...
      int k = 0;
      multAB[k].r =
          input_ftt_negate.points[0].histogram[0] * target_ftt.points[0].histogram[0];
      k++;
      ...
      multAB[nbins_ - 1].r = input_ftt_negate.points[0].histogram[nbins_ - 1] *
      ...
      for (int i = 0;
           (i <
            static_cast<int> (quantile * static_cast<float> (nr_bins_after_padding))) &&
           !stop;
           i++) {
