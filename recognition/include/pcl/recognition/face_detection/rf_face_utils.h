...
namespace pcl
{
  namespace face_detection
  {
        ...
        : public pcl::FeatureHandler<FT, DataSet, ExampleIndex>
    {
       ...
       */
      /*void createRandomFeatures(const size_t num_of_features, std::vector<FT> &
       features)
       {
       srand (time(NULL));
       int min_s = 10;
       ...
       * \param[in] examples The set of examples of the data set the feature is
       * evaluated on. \param[out] results The destination for the results of the
       * feature evaluation. \param[out] flags Flags that are supplied together with the
       * results.
       */
      void
                       ...
                       unsigned char &flag) const override
      {
        ...
        if (el_f1 == 0 || el_f2 == 0 ||
            (el_f1 <= static_cast<int> (f * static_cast<float> (feature.wsizex1_ *
                                                                feature.wsizey1_))) ||
            (el_f2 <= static_cast<int> (f * static_cast<float> (feature.wsizex2_ *
                                                                feature.wsizey2_)))) {
          result = static_cast<float> (
              pcl_round (static_cast<float> (rand ()) / static_cast<float> (RAND_MAX)));
          flag = 1;
        } else {
          result = static_cast<float> ((sum_f1 / static_cast<float> (el_f1) -
                                        sum_f2 / static_cast<float> (el_f2)) >
                                       feature.threshold_);
          flag = 0;
        }
      }

      /** \brief Generates evaluation code for the specified feature and writes it to
    ...
    /** \brief Statistics estimator for regression trees which optimizes information
     * gain and pose parameters error. */
        ...
        : public pcl::StatsEstimator<LabelDataType, NodeType, DataSet, ExampleIndex>
    {
                              ...
                              const float threshold) const override
      {
        ...
        float purity =
            sums[num_of_branches] /
            static_cast<LabelDataType> (branch_element_count[num_of_branches]);
        float tp = 0.8f;

        if (purity >= tp) {
          // compute covariance matrices from translation offsets and angles for the
          // whole set and children consider only positive examples...
          ...
          for (size_t branch_index = 0; branch_index < (num_of_branches + 1);
               ++branch_index) {
            hp[branch_index] +=
                std::max (sums[branch_index] / static_cast<float> (
                                                   branch_element_count[branch_index]) -
                              tp,
                          0.f) *
                hr[branch_index];
       ...
       * \param[in] examples The examples which define which parts of the data set are
       * used for evaluation. \param[in] label_data The label_data corresponding to the
