...
namespace pcl
{
   ...
   * "A Global Hypotheses Verification Method for 3D Object Recognition", A. Aldoma and
   * F. Tombari and L. Di Stefano and Markus Vincze, ECCV 2012 \author Aitor Aldoma
   */
      ...
      : public HypothesisVerification<ModelT, SceneT>
  {
    ...
    class SAModel : public mets::evaluable_solution
    {
      ...
      what_if (int /*index*/, bool /*val*/) const
      {
        ...
        tmp[index] = val;
        mets::gol_type sol = opt_->evaluateSolution (solution_, index); //evaluate
        without updating status return sol;*/
    ...
    {
      {
        ...
        for (const int &i : explained) {
          if (val < 0) {
            // the hypothesis is being removed, check that there are no points that
            // become unexplained and have clutter unexplained hypotheses
            if ((explained_by_RM[i] == 0) && (unexplained_by_RM[i] > 0)) {
              add_to_unexplained +=
                  unexplained_by_RM[i]; // the points become unexplained
            }
          } else {
            ...
            if ((explained_by_RM[i] == 1) &&
                (unexplained_by_RM[i] >
                 0)) { // the only hypothesis explaining that point
              add_to_unexplained -=
                           ...
                           float sign)
    {
      ...
      for (size_t i = 0; i < vec.size (); i++) {
        ...
        if ((explained_[vec[i]] > 1) &&
            prev_dup) { // its still a duplicate, we are adding
