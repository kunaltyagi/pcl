...
//----------------------------------------------------------------------------
void
vtkVertexBufferObjectMapper::Render (vtkRenderer *ren, vtkActor *act)
{
  ...
  // Draw
  ren->GetRenderWindow ()->GetPainterDeviceAdapter ()->DrawElements (
      VTK_VERTEX, indiceVbo->GetCount (), VTK_UNSIGNED_INT, nullptr);
  ...
  //
  //    ren->GetRenderWindow()->GetPainterDeviceAdapter()->DrawElements(VTK_VERTEX,
  //    normalIndiceVbo->GetCount(), VTK_UNSIGNED_INT, 0);
  //
  //    vertexVbo->UnBind();
  //    normalVbo->UnBind();
...
//----------------------------------------------------------------------------
void
vtkVertexBufferObjectMapper::SetInput (vtkPolyData *input)
{
...
void
vtkVertexBufferObjectMapper::SetInput (vtkDataSet *input)
{
  //  std::cout << "SetInput" << endl;
  if (input) {
    this->SetInputDataObject (0, input);
...
void
vtkVertexBufferObjectMapper::createVBOs (vtkRenderWindow *win)
{
  ...
  //  cout << "InterpolateScalarsBeforeMapping " <<
  //  this->InterpolateScalarsBeforeMapping << endl;
  ...
  // todo: This is VERY BAD!  We need to figure out where the scalar data is coming
  // from, ie PointData, CellData etc.
  //       Then use this as our color data.  In addition, scalars need to be mapped
  //       correctly via LUTs.  Although, writing a glsl shader for these mappings would
  //       probably be faster.
...
vtkPolyData *
vtkVertexBufferObjectMapper::GetInput ()
{
  return vtkPolyData::SafeDownCast (this->GetExecutive ()->GetInputData (0, 0));
}

void
vtkVertexBufferObjectMapper::Update ()
{
  cout << "Update" << endl;
...
void
vtkVertexBufferObjectMapper::ComputeBounds ()
{
...
//----------------------------------------------------------------------------
int
vtkVertexBufferObjectMapper::FillInputPortInformation (int vtkNotUsed (port),
                                                       vtkInformation *info)
{
