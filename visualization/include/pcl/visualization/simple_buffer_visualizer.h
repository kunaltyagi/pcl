...
namespace pcl
{
  namespace visualization
  {
     ...
     * pcl::VFHSignature308 for display. \note Therefore, the number of values is
     * limited to [2-308]. \author Raphael Favier \ingroup visualization
     */
    class PCL_EXPORTS PCLSimpleBufferVisualizer
    {
      ...
      PCLSimpleBufferVisualizer (const int nb_values)
      {
        ...
        if (nb_values_ > 308) {
          PCL_WARN ("Maximum number of values can only be 308 (%d given). Setting back "
                    "to 308. \n");
          nb_values_ = 308;
        }

        if (nb_values_ <= 1) {
          PCL_WARN ("Number of values must be at least 2 (%d given). Setting it to "
                    "default (308). \n");
          nb_values_ = 308;
        }
       ...
       * \note The point here is to change the background to orange when the latest
       * value is under a threshold. \param[in] value if true, automatic mode is
       * enabled. Else, background will be white \param[in] threshold value that
       * triggers the background to turn orange if the latest value is lower \note This
       * functionality does not work yet at time of commit (see
       ...
       * \note If set to false, the Y scale is automatically adjusted to the current
       * values (default). \param[in] value boolean that enable or disable this policy
       */
      void
      ...
      /** \brief boolean used to know if we need to change the background color in case
       * of low values. */
      ...
      /** \brief boolean used to know if we need to change the background color in case
       * of low values. True means we do it ourselves. */
