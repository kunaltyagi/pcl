...
class SimpleHDLGrabber
{
  ...
  sweepScan (const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &sweep)
  {
    ...
    if (++count == 30) {
      double now = pcl::getTime ();
      std::cout << "got sweep.  Avg Framerate " << double(count) / double(now - last)
                << " Hz" << std::endl;
      count = 0;
      last = now;
    }
  ...
  run ()
  {
    ...
    std::function<void(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr &, float, float)>
        f = boost::bind (&SimpleHDLGrabber::sectorScan, this, _1, _2, _3);
    ...
    std::function<void(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &)> f2 =
        boost::bind (&SimpleHDLGrabber::sweepScan, this, _1);
