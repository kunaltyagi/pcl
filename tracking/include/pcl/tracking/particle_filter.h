...
public:
   ...
   * \param[in] step_noise_covariance the diagonal elements of covariance matrix of step
   * noise.
   */
  inline void
  ...
  /** \brief Set the covariance of the initial noise. It will be used when initializing
   * the particles. \param[in] initial_noise_covariance the diagonal elements of
   * covariance matrix of initial noise.
   */
  inline void
  ...
  /** \brief Set the mean of the initial noise. It will be used when initializing the
   * particles. \param[in] initial_noise_mean the mean values of initial noise.
   */
  inline void
   ...
   * ParticleFilterTracker does not take the occluded points into account according to
   * the angle between the normal and the position. \param[in] occlusion_angle_thr
   * threshold of angle to be considered occlusion.
   */
  inline void
   ...
   * particles. \param[in] trans Affine transformation from the worldcoordinates to the
   ...
   * reference point cloud at the beginning and the best guess about its location in the
   * most recent frame.
   */
  inline StateT
  ...
  /** \brief Normalize the weight of a particle using \f$ exp(1- alpha ( w - w_{min}) /
   ...
   * serialized in change detection \param[in] change_detector_filter the minimum amount
   * of points required within leaf node
   */
  inline void
...
protected:
  ...
  /** \brief Crop the pointcloud by the bounding box calculated from hypothesis and the
   * reference pointcloud. \param[in] cloud a pointer to pointcloud to be cropped.
   ...
   *hypothesis a particle which represents a hypothesis. \param[in] indices the indices
   *which should be taken into account. \param[out] cloud the resultant point cloud
   *model dataset which is transformed to hypothesis.
   **/
  void
  ...
  /** \brief Initialize the particles. initial_noise_covariance_ and initial_noise_mean_
   * are used for Gaussian sampling. */
  void
  ...
  /** \brief An affine transformation from the world coordinates frame to the origin of
   * the particles. */
  ...
  /** \brief Pass through filter to crop the pointclouds within the hypothesis bounding
   * box. */
