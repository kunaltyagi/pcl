    ...
    class ParticleFilterTracker : public Tracker<PointInT, StateT>
    {
      ...
      ParticleFilterTracker ()
          : iteration_num_ (1), particle_num_ (), min_indices_ (1), ref_ (),
            particles_ (), coherence_ (), resample_likelihood_thr_ (0.0),
            occlusion_angle_thr_ (M_PI / 2.0), alpha_ (15.0), representative_state_ (),
            use_normal_ (false), motion_ (), motion_ratio_ (0.25), pass_x_ (),
            pass_y_ (), pass_z_ (), transed_reference_vector_ (), change_detector_ (),
            changed_ (false), change_counter_ (0), change_detector_filter_ (10),
       ...
       * \param[in] step_noise_covariance the diagonal elements of covariance matrix of
       * step noise.
       */
      inline void
       ...
       * initializing the particles. \param[in] initial_noise_covariance the diagonal
       * elements of covariance matrix of initial noise.
       */
      inline void
      ...
      /** \brief Set the mean of the initial noise. It will be used when initializing
       * the particles. \param[in] initial_noise_mean the mean values of initial noise.
       */
      inline void
       ...
       * ParticleFilterTracker does not take the occluded points into account according
       * to the angle between the normal and the position. \param[in]
      ...
      /** \brief Set the transformation from the world coordinates to the frame of the
       * particles. \param[in] trans Affine transformation from the worldcoordinates to
       * the frame of the particles.
       */
      inline void
      ...
      /** \brief Get the transformation from the world coordinates to the frame of the
       * particles. */
       ...
       * This function returns the particle that represents the transform between the
       * reference point cloud at the beginning and the best guess about its location in
       ...
       * frame. \param[in] particle an instance of StateT.
       */
      Eigen::Affine3f
      ...
      /** \brief Normalize the weight of a particle using \f$ exp(1- alpha ( w -
       * w_{min}) / (w_max - w_min)) \f$ \note This method is described in [P.Azad et.
       * al, ICRA11]. \param[in] w the weight to be normalized \param[in] w_min the
       * minimum weight of the particles \param[in] w_max the maximum weight of the
       * particles
       */
      inline double
      ...
      /** \brief Set the minimum amount of points required within leaf node to become
       * serialized in change detection \param[in] change_detector_filter the minimum
       * amount of points required within leaf node
       */
      inline void
      ...
      /** \brief Get the minimum amount of points required within leaf node to become
       * serialized in change detection. */
      inline unsigned int
       ...
       * cropped. \param[out] output a pointer to be assigned the cropped pointcloud.
       */
      void
       ...
       *represents. \param[in] hypothesis a particle which represents a hypothesis.
       * \param[in] indices the indices which should be taken into account.
       * \param[out] cloud the resultant point cloud model dataset which is transformed
       *to hypothesis.
       **/
      void
      ...
      /** \brief Compute a reference pointcloud transformed to the pose that hypothesis
       *represents and calculate indices taking occlusion into account. \param[in]
       *hypothesis a particle which represents a hypothesis. \param[in] indices the
       *indices which should be taken into account. \param[out] cloud the resultant
      ...
      /** \brief Weighting phase of particle filter method. Calculate the likelihood of
       * all of the particles and set the weights. */
      virtual void
      weight ();

      /** \brief Resampling phase of particle filter method. Sampling the particles
       * according to the weights calculated in weight method. In particular, "sample
       * with replacement" is archieved by walker's alias method.
       */
      virtual void
      resample ();
      ...
      /** \brief The diagonal elements of covariance matrix of the initial noise. the
       * covariance matrix is used when initialize the particles.
       */
      ...
      /** \brief An affine transformation from the world coordinates frame to the origin
       * of the particles. */
      ...
      typename pcl::octree::OctreePointCloudChangeDetector<PointInT>::Ptr
          change_detector_;
