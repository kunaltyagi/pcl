...
main (int argc, char *argv[])
{
  ...
  std::cout << "Filtered Pointcloud: " << doncloud->points.size () << " data points."
            << std::endl;
  std::stringstream ss;
  ...
  // Filter by magnitude
  cout << "Clustering using EuclideanClusterExtraction with tolerance <= " << segradius
       << "..." << endl;
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin ();
    ...
    std::cout << "PointCloud representing the Cluster: "
              << cloud_cluster_don->points.size () << " data points." << std::endl;
    std::stringstream ss;
    ss << outfile.substr (0, outfile.length () - 4) << "_threshold_" << threshold
       << "_cluster_" << j << ".pcd";
