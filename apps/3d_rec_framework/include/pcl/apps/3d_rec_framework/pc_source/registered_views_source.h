    ...
    class RegisteredViewsSource : public Source<PointInT>
    {
      ...
      loadOrGenerate (std::string &dir, std::string &model_path, ModelT &model)
      {
        ...
        if (bf::exists (trained_dir)) {
          ...
          pcl::PointCloud<PointInT>); pcl::transformPointCloud(*(model.views_->at(0)),
          *view_trans, view_transformation);

          pcl::visualization::PointCloudColorHandlerCustom<PointInT> random_handler2
          (view_trans, 0, 255, 0); vis.addPointCloud<PointInT> (view_trans,
          random_handler2, "view");
      ...
      /**
       * \brief Creates the model representation of the training set, generating views
       * if needed
       */
      void
