  ...
  namespace rec_3d_framework
  {
    ...
    class PreProcessorAndNormalEstimator
    {
                ...
                pcl::PointCloud<pcl::Normal>::Ptr &normals)
      {
        ...
        if (out->points.empty ()) {
          PCL_WARN ("NORMAL estimator: Cloud has no points after voxel grid, won't be "
                    "able to compute normals!\n");
          return;
        }
        ...
        if (remove_outliers_) {
          ...
          // thus creating a very sparse set of points that causes the normals to be
          // very noisy remove these points
        ...
        if (out->points.empty ()) {
          PCL_WARN (
              "NORMAL estimator: Cloud has no points after removing outliers...!\n");
          return;
        }
        ...
        if (out->isOrganized ()) {
          ...
          // check nans before computing normals
          {
            ...
            for (size_t i = 0; i < out->points.size (); ++i) {
              if (!std::isfinite (out->points[i].x) ||
                  !std::isfinite (out->points[i].y) ||
