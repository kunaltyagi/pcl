  ...
  for (int y = 0; y < static_cast<int> (image_->height); ++y) {
    for (int x = 0; x < static_cast<int> (image_->width); ++x) {
      ...
      if (hard_segmentation_[index] == SegmentationForeground) {
        // assert (static_cast<float>(GMM_component_[index]+1)/static_cast<float> (K_)
        // < 1.f);
        gmm_point.r =
...
main (int argc, char **argv)
{
  ...
  if (parsed_file_indices.empty ()) {
    pcl::console::print_error (
        "Need at least an input PCD file (e.g. scene.pcd) to continue!\n\n");
    pcl::console::print_info ("Ideally, need an input file, and two output PCD files, "
                              "e.g., object.pcd, background.pcd\n");
    return (-1);
  }
