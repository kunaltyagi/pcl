...
OutofcoreCloud::OutofcoreCloud (std::string name, boost::filesystem::path &tree_root)
    : Object (name), display_depth_ (1), points_loaded_ (0), data_loaded_ (0),
      render_camera_ (nullptr), lod_pixel_threshold_ (10000)
{
...
OutofcoreCloud::render (vtkRenderer *renderer)
{
  ...
  if (render_camera_ != nullptr && render_camera_->getName () == camera->getName ()) {
    ...
    while (*breadth_first_it != nullptr) {
      ...
      //        std::cout << " ";
      //      std::cout << coverage << "-" << pcd_file << endl;//" : " << (coverage >
      //      (size[0] * size[1])) << endl;
      ...
      if (cloud_data_cache.hasKey (pcd_file)) {
        ...
        if (!hasActor (cloud_actors_map_[pcd_file])) {
          points_loaded_ += cloud_actors_map_[pcd_file]
                                ->GetMapper ()
                                ->GetInput ()
                                ->GetNumberOfPoints ();
